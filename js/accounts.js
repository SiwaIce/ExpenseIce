...
const AccountsView = {
  ...
  openWalletModal(edit = null) {
    ...
    o.innerHTML = `<div class="modal"><h3>${isEdit ? '✏️ แก้ไขบัญชี' : '➕ เพิ่มบัญชี'}</h3>
      ...
      <div class="form-group"><label>ไอคอน</label><div style="display:flex;flex-wrap:wrap;gap:4px" id="waIconPick">${icons.map(ic => `<span style="font-size:1.3rem;cursor:pointer;padding:3px;border-radius:5px;border:2px solid ${isEdit && edit.icon === ic ? 'var(--accent)' : 'transparent'}" data-ic="${ic}">${ic}</span>`).join('')}</div><input type="text" id="waIcon" value="${isEdit ? edit.icon : '💵'}" maxlength="4" placeholder="หรือพิมพ์/วางอีโมจิเอง" style="width:120px;margin-top:6px;font-size:1.1rem;text-align:center"></div>
